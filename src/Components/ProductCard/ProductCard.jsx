...
const ProductCard = ({ item }) => {
    ...
    return (
        <motion.div
            whileHover={{ y: -8 }}
            className="flex h-[31rem] w-80 shrink-0 cursor-pointer select-none flex-col justify-center gap-4 overflow-hidden rounded-lg bg-white shadow-default transition  duration-100  ease-linear hover:shadow-hover"
        >
            ...
            <div className="flex flex-col items-center gap-4">
                <div className="flex max-w-52 flex-col gap-1 truncate text-center [&>p]:leading-none [&>p]:tracking-wide">
                    <p title={item.name} className="text-2xl font-extrabold uppercase tracking-wide">
                        {item.name}
                    </p>
